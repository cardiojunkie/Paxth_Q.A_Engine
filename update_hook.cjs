...
const hookFile = 'src/hooks/useCatalogData.ts';
...
export function useCatalogData() {
  ...
  const addParsedData = useCallback(async (data: SkuData[]) => {
    setSkuDataList((prev) => {
      ...
      data.forEach((item) => newMap.set(item.sku, item));
  ...
  return { skuDataList, addParsedData, updateSkuStatus, updateSku, removeSkus, isLoading };
}
`;
