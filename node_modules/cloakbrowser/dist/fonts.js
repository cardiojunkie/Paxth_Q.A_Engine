 ...
 */
export function countFontsPresent(tells) {
    let listing;
    try {
 ...
 */
export function windowsFontsPresent() {
 ...
 */
export function maybeWarnWindowsFonts(chromeArgs) {
    if (fontWarningChecked)
        return;
    fontWarningChecked = true;
    try {
        if (process.env.CLOAKBROWSER_SUPPRESS_FONT_WARNING)
            return;
        ...
        if (present === null || present === true)
            return; // full set present or undeterminable
        console.warn("[cloakbrowser] Incomplete Windows font set — installing the full set " +
