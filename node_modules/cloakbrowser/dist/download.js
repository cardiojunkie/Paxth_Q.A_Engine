...
import { BINARY_SIGNING_PUBKEYS, CHROMIUM_VERSION, DOWNLOAD_BASE_URL, GITHUB_API_URL, GITHUB_DOWNLOAD_BASE_URL, WRAPPER_VERSION, checkPlatformAvailable, getArchiveExt, getArchiveName, getBinaryDir, getBinaryPath, getCacheDir, getChromiumVersion, getDownloadUrl, getEffectiveVersion, getFallbackDownloadUrl, getLocalBinaryOverride, getPlatformTag, normalizeReleaseChannel, normalizeRequestedVersion, versionNewer, } from "./config.js";
 ...
 */
export async function ensureBinary(licenseKey, browserVersion, releaseChannel) {
    ...
    if (requestedVersion) {
        ...
        console.log(`[cloakbrowser] Stealth Chromium ${requestedVersion} not found. Downloading for ${getPlatformTag()}...`);
        await downloadAndExtract(requestedVersion);
        if (!fs.existsSync(binaryPath) || !isExecutable(binaryPath)) {
            throw new Error(`Pinned download completed but binary not found at expected path: ${binaryPath}. ` +
    ...
    if (!fs.existsSync(downloadedPath)) {
        throw new Error(`Download completed but binary not found at expected path: ${downloadedPath}. ` +
            `This may indicate a packaging issue. Please report at ` +
            `https://github.com/CloakHQ/cloakbrowser/issues`);
    }
    maybeTriggerUpdateCheck();
...
export async function checkForUpdate() {
    ...
    if (!latest || !versionNewer(latest, getChromiumVersion()))
        return null;
 ...
 */
export async function checkForProUpdate(licenseKey, releaseChannel) {
    ...
    if (!proBinaryReady(latest)) {
        ...
        if (!fs.existsSync(getBinaryPath(latest, true))) {
            throw new Error(`Pro download completed but binary not found at: ${getBinaryPath(latest, true)}`);
        }
    }
 ...
 * @internal Exported for testing only.
 */
export function welcomeDue(marker, pro) {
...
/** @internal Exported for testing only. */
export function resetPreviewFallbackWarned() {
 ...
 */
function warnPreviewFallback() {
    if (previewFallbackWarned)
        return;
    previewFallbackWarned = true;
    console.error(`[cloakbrowser] Preview channel requested, but no preview build is available for ${getPlatformTag()}; using the stable build.`);
}
function showWelcome(tier = "keyless") {
    const marker = path.join(getCacheDir(), ".welcome_shown");
    if (!welcomeDue(marker, tier === "pro"))
        return;
    console.error();
    ...
    console.error();
    if (tier === "pro") {
        console.error(`  CloakBrowser Pro active (v${PRO_MAJOR}) — latest binary, newest patches.`);
        console.error("  Pro support → support@cloakbrowser.dev");
    }
    else if (tier === "free") {
        console.error(`  CloakBrowser free (v${PRO_MAJOR}): the latest binary, 1 concurrent session.`);
        console.error("  For more than one concurrent session → https://cloakbrowser.dev");
    }
    else {
        const freeMajor = CHROMIUM_VERSION.split(".")[0];
        console.error(`  Running the free binary (v${freeMajor}). ` +
            `The latest binary (v${PRO_MAJOR}) is free too, with 1 concurrent session.`);
        console.error("  Get your key: run  cloakbrowser login  or visit https://cloakbrowser.dev/free");
        console.error("  For more than one concurrent session → https://cloakbrowser.dev");
    }
...
async function downloadAndExtract(version) {
    ...
    try {
        ...
        try {
            ...
            console.warn(`[cloakbrowser] Primary download failed (${primaryErr instanceof Error ? primaryErr.message : primaryErr}), trying GitHub Releases...`);
...
/** @internal Exported for testing only. */
export async function verifyDownloadChecksum(filePath, version) {
    ...
    if (process.env.CLOAKBROWSER_DOWNLOAD_URL) {
        ...
        if (process.env.CLOAKBROWSER_SKIP_CHECKSUM?.toLowerCase() === "true") {
            console.warn("[cloakbrowser] CLOAKBROWSER_SKIP_CHECKSUM set — skipping verification for custom download URL");
            return;
        }
        const checksums = await fetchChecksums(version);
        if (!checksums) {
            console.warn("[cloakbrowser] SHA256SUMS not available from custom URL — skipping checksum verification");
            return;
        }
    ...
    if (!manifest) {
        throw new Error("Could not fetch a signed SHA256SUMS (SHA256SUMS + SHA256SUMS.sig) for " +
    ...
    if (declared !== requested) {
        throw new Error(`Version mismatch in signed SHA256SUMS: requested ${requested}, ` +
    ...
    if (!expected) {
        throw new Error(`Signature-verified SHA256SUMS has no entry for ${tarballName} — ` +
 ...
 * @internal Exported for testing only.
 */
export async function fetchSignedManifest(version) {
 ...
 * @internal Exported for testing only.
 */
export function verifySignature(manifestBytes, sigB64Bytes) {
    ...
    throw new Error("SHA256SUMS signature verification failed — no pinned key validated the " +
        "manifest. The binary's authenticity could not be confirmed. " +
        "Report at https://github.com/CloakHQ/cloakbrowser/issues");
}
/** @internal Exported for testing only. */
export async function fetchChecksums(version) {
    const v = version || getChromiumVersion();
...
function proBinaryReady(version) {
    if (!version)
        return false;
...
function writeProVersionMarker(version, releaseChannel) {
    const cacheDir = getCacheDir();
...
async function ensureProBinary(licenseKey, requestedVersion, plan, releaseChannel) {
    ...
    if (requestedVersion) {
        ...
        console.log(`[cloakbrowser] Downloading Pro Chromium ${requestedVersion} for ${getPlatformTag()}...`);
    ...
    const frozen = (process.env.CLOAKBROWSER_AUTO_UPDATE ?? "").toLowerCase() === "false";
    ...
    try {
        console.log(`[cloakbrowser] Downloading Pro Chromium ${version} for ${getPlatformTag()}...`);
        ...
        if (err instanceof BinaryVerificationError)
            throw err;
        if (proBinaryReady(effective)) {
            console.log(`[cloakbrowser] Pro update to ${version} failed; launching cached Pro binary ${effective}`);
            showWelcome(welcomeTier);
            return getBinaryPath(effective, true);
        }
        throw err;
    ...
    if (!fs.existsSync(downloadedPath)) {
        throw new Error(`Pro download completed but binary not found at: ${downloadedPath}`);
    }
...
/** @internal Exported for testing only. */
export async function downloadProBinary(version, licenseKey) {
 ...
 * @internal Exported for testing only.
 */
export async function verifyProDownload(filePath, version) {
    ...
    try {
        ...
        if (!manifestResp.ok)
            throw new Error(`HTTP ${manifestResp.status} for SHA256SUMS`);
        const sigResp = await fetch(`${base}/SHA256SUMS.sig`, {
            redirect: "follow",
            signal: AbortSignal.timeout(10_000),
        });
        if (!sigResp.ok)
            throw new Error(`HTTP ${sigResp.status} for SHA256SUMS.sig`);
        manifestBytes = new Uint8Array(await manifestResp.arrayBuffer());
        sigBytes = new Uint8Array(await sigResp.arrayBuffer());
    }
    catch (e) {
    ...
    const declared = parseManifestVersion(manifestText);
    if (declared !== version) {
        throw new BinaryVerificationError(`Version mismatch in signed Pro SHA256SUMS: requested ${version}, ` +
            `manifest declares ${declared ?? "none"}. Refusing (possible downgrade).`);
    }
    const tarballName = getArchiveName();
    const expected = parseChecksums(manifestText).get(tarballName);
    if (!expected) {
        throw new BinaryVerificationError(`Signature-verified Pro SHA256SUMS has no entry for ${tarballName} — ` +
            `cannot confirm binary integrity.`);
    }
    try {
        await verifyChecksum(filePath, expected);
...
async function extractArchive(archivePath, destDir, binaryPath) {
...
function removeQuarantine(dirPath) {
    try {
...
/** @internal Exported for testing only. */
export async function getLatestChromiumVersion() {
    try {
...
/** @internal Exported for testing only. */
export function resetWrapperUpdateChecked() {
    wrapperUpdateChecked = false;
}
/** @internal Exported for testing only. */
export async function checkWrapperUpdate() {
    ...
    try {
        ...
        if (data.version && versionNewer(data.version, WRAPPER_VERSION)) {
            console.warn(`[cloakbrowser] Update available: ${WRAPPER_VERSION} → ${data.version}. ` +
...
async function checkAndDownloadUpdate() {
    try {
        ...
        const cacheDir = getCacheDir();
        fs.mkdirSync(cacheDir, { recursive: true });
        fs.writeFileSync(path.join(cacheDir, ".last_update_check"), String(Math.floor(Date.now() / 1000)));
        const platformVersion = getChromiumVersion();
        const latest = await getLatestChromiumVersion();
        ...
        console.log(`[cloakbrowser] Newer Chromium available: ${latest} (current: ${platformVersion}). Downloading in background...`);
        await downloadAndExtract(latest);
        writeVersionMarker(latest);
...
function maybeTriggerUpdateCheck() {
