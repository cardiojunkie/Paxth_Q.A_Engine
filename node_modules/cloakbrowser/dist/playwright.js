 ...
 */
import { DEFAULT_VIEWPORT, IGNORE_DEFAULT_ARGS, binarySupportsHeadlessNoViewport, } from "./config.js";
...
import { buildLaunchEnv, installLicenseGuard, licenseErrorFrom, mintDenialFile, resolveLicenseKey, } from "./license.js";
 ...
 */
function filterStealthCtxOptions(ctx) {
    if (!ctx)
        return {};
...
function effectiveHeadless(options) {
    return (options.launchOptions?.headless ??
        options.headless ??
        true);
}
export function buildContextOptions(options = {}) {
    ...
    const headlessNoViewport = headless && binarySupportsHeadlessNoViewport(options.licenseKey, options.browserVersion, options.releaseChannel);
 ...
 */
export async function buildLaunchOptions(options = {}, statusFile) {
    const binaryPath = process.env.CLOAKBROWSER_BINARY_PATH ||
        (await ensureBinary(options.licenseKey, options.browserVersion, options.releaseChannel));
    const { exitIp, ...resolved } = await maybeResolveGeoip(options);
    const { proxyOption, proxyArgs } = resolveProxyConfig(options.proxy, options.browserVersion, options.licenseKey, options.releaseChannel);
    ...
    const launchEnv = buildLaunchEnv(options.licenseKey, userEnv, statusFile);
 ...
 */
export async function humanizeBrowser(browser, options = {}) {
    if (!options.humanize)
        return;
...
export async function launch(options = {}) {
    ...
    if (denialPath) {
        browser.__cloakDenialPath = denialPath;
    ...
    if (!effectiveHeadless(options) ||
        binarySupportsHeadlessNoViewport(options.licenseKey, options.browserVersion, options.releaseChannel)) {
 ...
 */
function applyDefaultNoViewport(browser) {
    const origNewContext = browser.newContext.bind(browser);
    browser.newContext = (options) => origNewContext(options?.viewport === undefined ? { ...options, viewport: null } : options);
    const origNewPage = browser.newPage.bind(browser);
    browser.newPage = (options) => origNewPage(options?.viewport === undefined ? { ...options, viewport: null } : options);
}
/**
...
export async function launchContext(options = {}) {
    ...
    const denialPath = browser.__cloakDenialPath;
    if (denialPath) {
...
export async function launchPersistentContext(options) {
    ...
    const denialPath = resolveLicenseKey(options.licenseKey) ? mintDenialFile() : undefined;
    const { env: userEnv, ...restLaunchOptions } = options.launchOptions ?? {};
    const launchEnv = buildLaunchEnv(options.licenseKey, userEnv, denialPath);
    const envResult = launchEnv !== undefined ? { env: launchEnv } : {};
