...
function patchPage(page, cfg, cursor) {
    ...
    // --- mouse patches ---
    page.mouse.move = async (x, y, _options) => {
        await ensureCursorInit();
        await humanMove(raw, cursor.x, cursor.y, x, y, cfg);
        cursor.x = x;
        cursor.y = y;
    };
    page.mouse.click = async (x, y, _options) => {
        await ensureCursorInit();
        await humanMove(raw, cursor.x, cursor.y, x, y, cfg);
        cursor.x = x;
        cursor.y = y;
    ...
    // --- keyboard patches ---
    page.keyboard.type = async (text, _options) => {
        const cdp = await ensureCdp();
...
function patchFrames(page, cfg, cursor, raw, rawKb, originals, stealth) {
    ...
    if (!page._humanFrameListenerAttached) {
        ...
        page._humanFrameListenerAttached = true;
    }
...
function patchSingleFrame(frame, page, cfg, cursor, raw, rawKb, originals, stealth) {
    ...
    if (frame === page.mainFrame()) {
        const p = page;
        frame.click = (selector, options) => p.click(selector, options);
        frame.dblclick = (selector, options) => p.dblclick(selector, options);
        frame.hover = (selector, options) => p.hover(selector, options);
        frame.type = (selector, text, options) => p.type(selector, text, options);
        frame.fill = (selector, value, options) => p.fill(selector, value, options);
        frame.check = (selector, options) => p.check(selector, options);
        frame.uncheck = (selector, options) => p.uncheck(selector, options);
        frame.selectOption = (selector, values, options) => p.selectOption(selector, values, options);
        frame.press = (selector, key, options) => p.press(selector, key, options);
        frame.pressSequentially = (selector, text, options) => p.pressSequentially(selector, text, options);
        frame.tap = (selector, options) => p.tap(selector, options);
        frame.clear = (selector, options) => p.clear(selector, options);
