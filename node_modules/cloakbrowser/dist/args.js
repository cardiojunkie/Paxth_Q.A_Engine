...
export function buildArgs(options) {
    ...
    if (options.args) {
        for (const arg of options.args) {
            ...
            if (seen.has(key)) {
                if (DEBUG)
                    console.debug(`[cloakbrowser] Arg override: ${seen.get(key)} -> ${arg}`);
            }
            seen.set(key, arg);
        }
    }
    ...
    if (options.timezone) {
        ...
        if (seen.has(key)) {
            if (DEBUG)
                console.debug(`[cloakbrowser] Arg override: ${seen.get(key)} -> ${flag}`);
        }
        seen.set(key, flag);
    }
    if (options.locale) {
        for (const k of ["--lang", "--fingerprint-locale"]) {
            const flag = `${k}=${options.locale}`;
            if (seen.has(k)) {
                if (DEBUG)
                    console.debug(`[cloakbrowser] Arg override: ${seen.get(k)} -> ${flag}`);
            }
            seen.set(k, flag);
        }
    }
    ...
    const explicitViewport = options.viewport !== undefined ||
        options.launchOptions?.defaultViewport !== undefined;
    const hasWindowFlag = ["--start-maximized", "--window-size", "--window-position"].some(k => seen.has(k));
    if (!explicitViewport &&
        !hasWindowFlag &&
        binarySupportsMaximizedWindow(options.licenseKey, options.browserVersion, options.releaseChannel)) {
