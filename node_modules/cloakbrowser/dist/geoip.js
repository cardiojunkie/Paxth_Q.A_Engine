...
import { ensureProxyScheme, isSocksProxy, reconstructHttpUrl, reconstructSocksUrl } from "./proxy.js";
...
export async function resolveProxyGeo(proxyUrl) {
    ...
    if (!ip && proxyUrl && !deadlineExpired(deadline))
...
function deadlineFromTimeout(timeoutMs) {
...
async function resolveExitIp(proxyUrl, timeoutMs) {
            ...
            try {
                const ip = await new Promise((resolve, reject) => {
                    ...
                    connectReq.on("connect", (_res, socket) => {
                        ...
                        const req = https.request(echoUrl, 
                        ...
                        { socket, agent: false, timeout: Math.min(5_000, innerRemaining ?? 5_000) }, (res) => {
                            let data = "";
                            res.on("data", (chunk) => (data += chunk.toString()));
                            res.on("end", () => {
                                const ip = data.trim();
...
let geoipDownloadPromise = null;
function runGuardedDownload(dbPath) {
    if (geoipDownloadPromise)
 ...
 */
export function appendWebrtcExitIp(args, exitIp) {
 ...
 */
export async function resolveWebrtcArgs(options) {
