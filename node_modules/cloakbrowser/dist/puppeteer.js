 ...
 */
import { DEFAULT_VIEWPORT, IGNORE_DEFAULT_ARGS, binarySupportsHeadlessNoViewport, binarySupportsHttpProxyInlineAuth, } from "./config.js";
 ...
 */
function resolveDefaultViewport(options) {
    const launchOpts = (options.launchOptions ?? {});
    ...
    if (launchOpts.defaultViewport !== undefined) {
        return launchOpts.defaultViewport;
    }
 ...
 */
function resolveProxy(options, args) {
    if (!options.proxy)
        return undefined;
    if (isSocksProxy(options.proxy)) {
        const { proxyArgs } = resolveProxyConfig(options.proxy, options.browserVersion, options.licenseKey, options.releaseChannel);
        args.push(...proxyArgs);
        return undefined;
    }
    // On binaries that ship inline proxy auth: pass full URL with inline creds.
    if (binarySupportsHttpProxyInlineAuth(options.licenseKey, options.browserVersion, options.releaseChannel)) {
...
async function applyPostLaunch(browser, options, proxyAuth) {
    if (proxyAuth) {
