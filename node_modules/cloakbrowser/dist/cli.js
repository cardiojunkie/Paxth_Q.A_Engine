...
import { getLocalBinaryOverride, getCacheDir, getPlatformTag, getBinaryPath, getBinaryDir, getEffectiveVersion, normalizeReleaseChannel, normalizeRequestedVersion, versionNewer, CHROMIUM_VERSION, WRAPPER_VERSION, } from "./config.js";
import { countFontsPresent, WINDOWS_FONT_TELLS, OFFICE_FONT_TELLS } from "./fonts.js";
import { resolveProxyGeo } from "./geoip.js";
import { resolveLicenseKey, validateLicense, getProLatestRelease, getActiveSessionCount } from "./license.js";
...
const FREE_LATEST_HINT = "Get the latest binary free → run 'cloakbrowser login' or https://cloakbrowser.dev/free";
...
function missingSharedLibs(binaryPath) {
...
async function resolveLicense() {
    ...
    if (process.env.CLOAKBROWSER_DOWNLOAD_URL)
        key = undefined;
    if (!key)
        return { license: { tier: "free" }, entitledPro: false };
    try {
        const lic = await validateLicense(key);
        if (lic === null)
            return { license: { tier: "unknown", error: "could not validate" }, entitledPro: false };
        if (lic.valid)
            return { license: { tier: lic.plan, valid: true, expires: lic.expires }, entitledPro: true };
        return { license: { tier: "invalid", valid: false }, entitledPro: false };
    }
    catch (err) {
        return { license: { tier: "unknown", error: err.message }, entitledPro: false };
    }
}
/** Describe the binary ensureBinary would actually launch (no download). */
async function effectiveBinary(entitledPro, quick = false) {
...
export async function collectDiagnostics(quick, proxy) {
    const diag = {};
    diag.environment = {
    ...
    try {
        ...
        diag.environment.platform_tag = `unavailable (${err.message})`;
    }
    try {
        diag.binary = await effectiveBinary(entitledPro, quick);
    }
    catch (err) {
        diag.binary = { error: err.message };
    }
    // Launch test (skipped by --quick or when the binary is not installed).
    const binPath = diag.binary.path;
    const installed = diag.binary.installed;
    if (quick) {
        ...
        if (!ok)
            diag.launch.missing_libs = missingSharedLibs(binPath);
    }
    ...
    if (proxy) {
        try {
            ...
            diag.geoip.resolved = { error: err.message };
        }
    }
    // Optional peer deps.
    diag.modules = {
...
function printDiagnostics(diag) {
    ...
    if (binary.error) {
        ...
        if (binary.tier === "override") {
            ...
            if (binary.latest_version) {
                ...
                if (binary.latest_version === binary.version && binary.installed) {
                    ...
                    console.log(`Latest:    ${binary.latest_version} (available — pinned; unset CLOAKBROWSER_VERSION to upgrade)`);
                }
                else {
                ...
                console.log(`Version:   not downloaded yet (${binary.tier}) — next launch downloads the latest`);
            }
            else {
        ...
        if (binary.cache_dir)
            console.log(`Cache:     ${binary.cache_dir}`);
        if (binary.override) {
    ...
    if (lic.sessions) {
        const active = lic.sessions.active;
...
async function promptFreeGithubLogin() {
...
async function cmdLogin(rest) {
    ...
    if (!key) {
        if (!process.stdin.isTTY) {
            console.error("Usage: cloakbrowser login <key>  (or run it interactively for a free key).");
            process.exit(2);
        }
        const entered = await promptLine("Paste your license key, or press ENTER to get a free key via GitHub: ");
    ...
    if (info === null) {
        console.error("Could not reach the license server to verify the key. Check your connection and retry.");
        process.exit(1);
    }
    if (!info.valid) {
    ...
    saveLicenseKey(key);
    if (info.plan === "free") {
        console.log("Saved. Free tier active: latest binary, 1 concurrent session (one browser at a time).");
...
function cmdLogout() {
    ...
    if (fs.existsSync(keyFile)) {
        try {
            ...
            console.error(`Could not remove ${keyFile}: ${err.message}`);
            process.exit(1);
        }
