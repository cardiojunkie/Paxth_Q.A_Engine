...
function patchPage(page, cfg, cursor) {
    ...
    if (originals.mouseDragAndDrop) {
        page.mouse.dragAndDrop = async (start, target, _options) => {
            await ensureCursorInit();
    ...
    page.keyboard.press = async (key, _options) => {
...
function patchSingleElementHandle(el, page, cfg, cursor, raw, rawKb, originals, stealth) {
    ...
    const origElScrollIntoView = el.scrollIntoView?.bind(el);
    // --- Nested selectors ---
    ...
    const moveToElement = async (callCfg = cfg) => {
        ...
        try {
            const { cursorX, cursorY } = await humanScrollIntoView(page, raw, () => el.boundingBox().then(b => b ?? null), cursor.x, cursor.y, callCfg);
            cursor.x = cursorX;
            cursor.y = cursorY;
        }
    ...
    if (origElScrollIntoView) {
        el.scrollIntoView = async (options) => {
            const callCfg = mergeConfig(cfg, options?.human_config ?? options);
            await page._ensureCursorInit();
            try {
                const { cursorX, cursorY } = await humanScrollIntoView(page, raw, () => el.boundingBox().then(b => b ?? null), cursor.x, cursor.y, callCfg);
