 ...
 */
function assembleSocksUrl(scheme, encUser, encPass, hostAndRest) {
    let userinfo;
    if (encPass !== null) {
 ...
 */
function lenientDecodeURIComponent(s) {
    return s.replace(/%([0-9A-Fa-f]{2})|%/g, (_match, hex) => hex ? String.fromCharCode(parseInt(hex, 16)) : "%");
}
/**
 ...
 */
export function normalizeSocksStringUrl(urlStr) {
    ...
    try {
        ...
        if (credsChanged) {
            console.info("[cloakbrowser] Auto URL-encoded SOCKS5 proxy credentials (special " +
                "characters detected). Pre-encode the URL to suppress this notice.");
        }
        return normalized;
    }
    catch (e) {
        console.warn(`[cloakbrowser] Could not normalize SOCKS5 proxy URL, passing through unchanged: ${e.message}`);
        return urlStr;
    }
}
 ...
 */
export function reconstructHttpUrl(proxy) {
    if (!proxy.username)
        return proxy.server;
 ...
 */
export function normalizeHttpStringUrl(urlStr) {
    ...
    try {
        ...
        const credsChanged = encUser !== rawUserEnc
            || (hasPassword ? encPass !== rawPassEnc : false);
        if (credsChanged) {
            console.info("[cloakbrowser] Auto URL-encoded HTTP proxy credentials (special " +
                "characters detected). Pre-encode the URL to suppress this notice.");
        }
        return result;
    }
    catch (e) {
        console.warn(`[cloakbrowser] Could not normalize HTTP proxy URL, passing through unchanged: ${e.message}`);
        return normalized;
    }
}
/**
 ...
 */
export function resolveProxyConfig(proxy, browserVersion, licenseKey, releaseChannel) {
    if (!proxy)
        return { proxyArgs: [] };
